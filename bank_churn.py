...
piv=df.groupby(['Exited','Gender', 'Geography']).agg({'NumOfProducts': ['median'],'EstimatedSalary': ['median'],
                                         'Age': ['median', 'count']}).copy()

st.write("### Аналитика ухода клиентов из банка:")
st.write("0- клиент остался, 1 - клиент ушел")
st.dataframe(piv)
st.write("### Вывод:")
st.write("Как видно из сводной таблицы, которая была сгруппирована по среднему возрасту, полу, стране резидентства клиента, его зп и количеству используемых сервисов банка"
"- женщины из Франции и Германии лидируют,они чаще всего отказывались от услуг банка в возрасте за 40 лет")
